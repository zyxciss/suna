...
export function HtmlRenderer({
    ...
    const blobHtmlUrl = useMemo(() => {
        if (content && !project?.sandbox?.id) {
            const blob = new Blob([content], { type: 'text/html' });
            return URL.createObjectURL(blob);
        }
        return undefined;
    }, [content, project?.sandbox?.id]);
    ...
    const htmlPreviewUrl = useMemo(() => {
        if (project?.sandbox?.id && fileName) {
            return constructHtmlPreviewUrl(project.sandbox.id, fileName);
        }
        return blobHtmlUrl || previewUrl;
    }, [project?.sandbox?.id, fileName, blobHtmlUrl, previewUrl]);
